...
4. T-15 短动量入口窗口：15~9 秒内检查；BTC 偏离 `>=5 bps`，最近 3 秒同向，最近 5 秒同向 `>=0.5 bps`，ask `<=0.98`，spread `<=0.10`。
5. T-8 强信号窗口：8~6 秒内检查；BTC 偏离 `>=5 bps`，`0.5 <= ask <= 0.95`，可用 100% 当盘预算。
6. T-8 普通信号窗口：8~6 秒内检查；BTC 偏离 `>=0.5 bps`，`0.5 <= ask <= 0.93`，可用 75% 当盘预算。
7. T-5 强 fallback 窗口：5~4 秒内检查；BTC 偏离 `>=1.5 bps`，`0.5 <= ask <= 0.98`，spread `<=0.10`，可用 100% 当盘预算。
8. T-5 低价 fallback 窗口：5~4 秒内检查；BTC 偏离 `>=0.2 bps`，`0.5 <= ask <= 0.85`，spread `<=0.10`，可用 75% 当盘预算。
9. T-3 最后 fallback 窗口：3~1 秒内检查；`0.5 <= ask <= 0.95`，可用 100% 当盘预算；不吃 0.97~0.99 高价尾盘，也不买盘口明显不认可的一边。
...
- `BTC_ORACLE_PROFILE`: 六层 fallback 配置；机器人按顺序检查当前时间窗口内的 tier，第一档满足就下单。
- `label`: `jytd` 和信号日志里显示的阶段名。
- `sec`: 窗口起点秒数，不再是单一秒点。窗口下界由下一个更低的 `sec` 决定；默认 `15/8/5/3` 会形成 `15~9`、`8~6`、`5~4`、`3~1`。
...
- `Bot::decide_btc_oracle_fallback`: Candidate C，BTC 涨跌幅、短动量和 ask 盈亏比分层 fallback。
...
Candidate C 诊断命令。部署后安装为 `/usr/local/bin/jydiag`。默认读取 `/opt/mybot-codex/.env`、state 和 signals，输出最近 24 小时触发/拦截/缺盘口/延迟统计。
